...
def make_runtime_request(
    ...
    try:
        ...
        print("❌ Error: Could not connect to TAILER backend at {BASE_URL}")
...
def test_invalid_key():
    ...
    if status_code == 401:
        print(f"\n✅ CORRECTLY REJECTED (HTTP {status_code})")
        print(f"   Error: {response.get('detail')}")
    else:
        print(f"\n❌ Unexpected response (HTTP {status_code})")
...
def test_all_keys():
    ...
    for team, key_info in DEMO_KEYS.items():
        ...
        if response and status_code == 200:
            print(f"  ✅ Success - {response.get('model')} responded")
        else:
